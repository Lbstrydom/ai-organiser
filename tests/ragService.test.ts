...
describe('RAGService', () => {
    ...
    describe('retrieveContext', () => {
        ...
        it('should filter low-similarity results', async () => {
            // With very high threshold, should get fewer results than default
            const defaultContext = await ragService.retrieveContext('learning', undefined, {});
            const strictContext = await ragService.retrieveContext('learning', undefined, {
                minSimilarity: 0.99
            });

            // Strict threshold should return fewer or equal results
            expect(strictContext.chunks.length).toBeLessThanOrEqual(defaultContext.chunks.length);
        });
