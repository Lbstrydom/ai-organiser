 ...
 * 1. SPECIFICATION TESTS (MockDictionaryService): Document expected parsing/formatting behavior
 *    - These test a local replica of the parsing logic
 *    - Purpose: Define the contract for how dictionaries should be parsed
 * 2. PRODUCTION TESTS (DictionaryService): Test actual production code
...
// These tests verify the actual DictionaryService behavior
// ============================================================================

describe('DictionaryService (Production)', () => {
    let mockApp: App;
    let service: DictionaryService;

    beforeEach(() => {
        mockApp = new App();
        service = new DictionaryService(mockApp as any);
    });
    ...
    describe('createEmptyDictionary', () => {
        ...
        it('should set timestamps', () => {
            ...
            expect(() => new Date(dict.createdAt)).not.toThrow();
        });
    });
...
Let me know if you need more.`;
    ...
    describe('listDictionaries (with mocked vault)', () => {
        ...
        it('should parse dictionaries from vault files', async () => {
            const mockFolder = new TFolder('AI-Organiser/Config/dictionaries');
            const mockFile = new TFile('AI-Organiser/Config/dictionaries/test-dict.md');
...
`;
            ...
            mockApp.vault.getAbstractFileByPath = (path: string) => {
                if (path.includes('dictionaries') && !path.endsWith('.md')) return mockFolder;
                if (path.endsWith('.md')) return mockFile;
                return null;
            };
            ...
            expect(dictionaries[0].name).toBe('Test Dictionary');
            expect(dictionaries[0].entries.length).toBe(1);
            expect(dictionaries[0].entries[0].term).toBe('John Smith');
        });
    });

    describe('loadDictionary (with mocked vault)', () => {
        it('should return null when file does not exist', async () => {
            mockApp.vault.getAbstractFileByPath = () => null;

            const dictionary = await service.loadDictionary('nonexistent.md');

            expect(dictionary).toBeNull();
        });

        it('should parse dictionary from file content', async () => {
            const mockFile = new TFile('test-dict.md');
            const fileContent = `---
name: Test Dictionary
description: Testing
---
...
- **API** - Application Programming Interface
- **KPI** (Key Perf) - Key Performance Indicator
`;

            mockApp.vault.getAbstractFileByPath = () => mockFile;
            mockApp.vault.read = async () => fileContent;

            const dictionary = await service.loadDictionary('test-dict.md');

            expect(dictionary).not.toBeNull();
            expect(dictionary!.name).toBe('Test Dictionary');
            expect(dictionary!.entries.length).toBe(2);
            expect(dictionary!.entries[0].term).toBe('API');
            expect(dictionary!.entries[0].category).toBe('acronym');
            expect(dictionary!.entries[1].aliases).toContain('Key Perf');
        });
    });

    describe('saveDictionary (with mocked vault)', () => {
        it('should generate markdown content and save to vault', async () => {
            let savedContent = '';
            let savedPath = '';
            ...
            dict.entries = [
                { term: 'Test Term', category: 'term', definition: 'A test' }
            ];

            await service.saveDictionary(dict);

            expect(savedPath).toContain('new-dictionary.md');
            expect(savedContent).toContain('name: New Dictionary');
            expect(savedContent).toContain('## Terms');
            expect(savedContent).toContain('- **Test Term** - A test');
        });
    });
});
