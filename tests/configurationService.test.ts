...
import { App, createTFile } from './mocks/obsidian';
import {
...
describe('ConfigurationService', () => {
    ...
    describe('Persona Parsing via Production Paths', () => {
        function mockPersonasFile(content: string, kind: 'minutes' | 'summary' | 'writing' = 'minutes') {
            const file = createTFile(`AI-Organiser/Config/${kind}-personas.md`);
...
`);
        ...
        it('parses personas whose prompts contain ### headers (code-fence-aware)', async () => {
            mockPersonasFile(`# Personas

### Study [icon: graduation-cap]

> Academic study notes

\`\`\`\`
**Role:** Master note-taker
...
### Review Checklist
- [ ] Can I explain the core claim?
\`\`\`\`

### Brief (default) [icon: zap]

> Quick summary

\`\`\`\`
**Role:** Concise briefer
...
`);
            ...
            expect(personas).toHaveLength(2);
            expect(personas[0].name).toBe('Study');
            expect(personas[0].prompt).toContain('### Big Picture');
            expect(personas[0].prompt).toContain('### Terminology');
            expect(personas[0].prompt).toContain('### Review Checklist');
            expect(personas[1].name).toBe('Brief');
            expect(personas[1].isDefault).toBe(true);
            expect(personas[1].prompt).toContain('### The Lede');
            expect(personas[1].prompt).toContain('### The Bottom Line');
        });

        it('parses personas with inner 3-backtick code blocks (mermaid etc.)', async () => {
            mockPersonasFile(`# Personas

### Feynman [icon: lightbulb]

> Layered explanation with diagrams
...
`);
            ...
            expect(personas).toHaveLength(1);
            expect(personas[0].name).toBe('Feynman');
            expect(personas[0].prompt).toContain('### Layer 1');
            expect(personas[0].prompt).toContain('### Layer 2');
            expect(personas[0].prompt).toContain('```mermaid');
            expect(personas[0].prompt).toContain('### Layer 3');
        });

        it('handles old 3-backtick format for simple prompts (backward compat)', async () => {
            mockPersonasFile(`# Personas
...
`);
            ...
            expect(personas[0].prompt).toBe('Be concise and clear.');
            expect(personas[1].prompt).toBe('Be thorough.');
        });

        it('round-trips default summary personas through generate+parse', async () => {
            // Generate the file content, then parse it back
            // This validates that the generator and parser work together
            const generatedContent = (service as any).generateSummaryPersonasFileContent();

            mockPersonasFile(generatedContent, 'summary');

            const personas = await service.getSummaryPersonas();

            expect(personas).toHaveLength(DEFAULT_SUMMARY_PERSONAS.length);
            for (const defaultPersona of DEFAULT_SUMMARY_PERSONAS) {
                const parsed = personas.find(p => p.name === defaultPersona.name);
                expect(parsed).toBeDefined();
                expect(parsed!.prompt).toBe(defaultPersona.prompt);
                expect(parsed!.description).toBe(defaultPersona.description);
            }
        });

        it('round-trips default writing personas through generate+parse', async () => {
            const generatedContent = (service as any).generatePersonasFileContent();

            mockPersonasFile(generatedContent, 'writing');

            const personas = await service.getPersonas();

            expect(personas).toHaveLength(DEFAULT_PERSONAS.length);
    ...
    describe('Caching', () => {
        it('does not re-read config within the cache TTL', async () => {
            const file = createTFile('AI-Organiser/Config/minutes-personas.md');
            let readCount = 0;
    ...
    describe('migratePersonaConfigFiles', () => {
        ...
        it('overwrites file when old version marker is present', async () => {
            ...
            app.vault.getAbstractFileByPath = (path: string) => {
                if (path.endsWith('-personas.md')) return createTFile(path);
                return null;
            };
