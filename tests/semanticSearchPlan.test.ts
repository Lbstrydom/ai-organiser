...
describe('AIOrganiserPlugin.saveSettings', () => {
    ...
    it('clears index when embedding settings change', async () => {
        ...
        const rebuildVault = vi.fn().mockResolvedValue({ indexed: 0, failed: 0 });
        plugin.vectorStoreService = { updateEmbeddingService, rebuildVault } as any;
...
describe('INDEX_SCHEMA_VERSION consistency', () => {
    ...
    it('follows semver format', () => {
        expect(INDEX_SCHEMA_VERSION).toMatch(/^\d+\.\d+\.\d+$/);
    });
});
