...
import { App, Platform, createTFile } from './mocks/obsidian';
...
describe('path integration', () => {
    ...
    it('NotebookLM resolves legacy pluginFolder prefix under split output root', async () => {
        const commands: any[] = [];
        const getAbstractFileByPath = vi.fn(() => ({}));
        ...
        await openCommand.callback();
        // Legacy 'MyPlugin/NotebookLM' should resolve under 'My Output', not 'MyPlugin'
        expect(getAbstractFileByPath).toHaveBeenCalledWith('My Output/NotebookLM');
    });

    it('Minutes modal uses the dictionaries folder under the resolved config path', async () => {
        const app = new App();
        const file = createTFile('MyPlugin/Config/dictionaries/team-glossary.md');
