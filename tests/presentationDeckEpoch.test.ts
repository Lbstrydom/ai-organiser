...
function makeHandler(): PresentationModeHandler & Internals {
    const h = new PresentationModeHandler() as PresentationModeHandler & Internals;
    h.html = '<deck>';
    h.deckIr = coffeeDeckIr;
    h.activeSlideIndex = 0;
    return h;
}
...
describe('deck epoch — monotonic mutation counter', () => {
    ...
    it('keeps increasing past the MAX_VERSIONS cap (length stops growing, epoch does not)', () => {
        ...
        for (let i = 0; i < pushes; i++) h.pushVersion(`v${i}`);
        // versions array is capped...
        expect(h.versions.length).toBe(MAX_VERSIONS);
