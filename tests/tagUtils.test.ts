...
describe('TagUtils.mergeTags', () => {
  it('should merge two arrays of tags with priority tags first', () => {
    const priority = ['ai', 'ml'];
    const supplementary = ['tech', 'science'];
    const result = TagUtils.mergeTags(priority, supplementary);
    expect(result).toEqual(['ai', 'ml', 'tech', 'science']);
  });

  it('should remove duplicates case-insensitively, keeping priority version', () => {
    const priority = ['Tech', 'ai'];
    const supplementary = ['tech', 'science'];
    const result = TagUtils.mergeTags(priority, supplementary);
    expect(result.filter(t => t.toLowerCase() === 'tech').length).toBe(1);
    expect(result).toEqual(['Tech', 'ai', 'science']);
  });

  it('should preserve priority tag ordering (theme/discipline/topics)', () => {
    const priority = ['mango'];
    const supplementary = ['zebra', 'apple'];
    const result = TagUtils.mergeTags(priority, supplementary);
    expect(result).toEqual(['mango', 'zebra', 'apple']);
  });
  ...
  it('should preserve head positions for taxonomy tags during merge', () => {
    const newTags = ['technology', 'computer-science', 'neural-networks', 'deep-learning'];
    const existingTags = ['old-topic', 'another-topic'];
    const result = TagUtils.mergeTags(newTags, existingTags);
    expect(result[0]).toBe('technology');
    expect(result[1]).toBe('computer-science');
    expect(result).toEqual(['technology', 'computer-science', 'neural-networks', 'deep-learning', 'old-topic', 'another-topic']);
  });
});
