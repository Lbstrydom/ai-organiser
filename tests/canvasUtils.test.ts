...
describe('Canvas Utils', () => {
    ...
    it('generateId should produce 16-char lowercase hex matching Obsidian convention', () => {
        for (let i = 0; i < 100; i++) {
            const id = generateId();
    ...
    it('buildCanvasEdge should generate hex-format edge IDs', () => {
        ...
        expect(canvasEdge.id).toMatch(/^[0-9a-f]{16}$/);
    });

    it('generateId should fall back gracefully without Web Crypto API', () => {
        ...
        try {
            ...
            for (let i = 0; i < 50; i++) {
                fallbackIds.add(generateId());
            }
            expect(fallbackIds.size).toBe(50);
        } finally {
