...
import { parseStructuredResponse, sanitizeSummaryHook, extractPlainText, tryParseJson, tryParseJsonFromFence, tryParseJsonFromObject, tryExtractJson, splitCompanionContent, repairJsonStrings } from '../src/utils/responseParser';
...
Hope this helps!`;
    ...
    describe('Tier 4: Repair Literal Newlines in JSON Strings', () => {
        it('should parse JSON with literal newlines in body_content', () => {
            // Simulate LLM outputting JSON with real newlines inside string values
            const response = '{\n  "summary_hook": "A brief hook",\n  "body_content": "## Heading\n\nParagraph one.\n\nParagraph two.",\n  "suggested_tags": ["test"],\n  "content_type": "note",\n  "detected_language": "en"\n}';

            const result = parseStructuredResponse(response);

            expect(result).not.toBeNull();
            expect(result?.body_content).toContain('## Heading');
            expect(result?.body_content).toContain('Paragraph two.');
            expect(result?.body_content).not.toContain('"summary_hook"');
        });

        it('should parse JSON with literal tabs in string values', () => {
            const response = '{\n  "summary_hook": "Hook",\n  "body_content": "Col1\tCol2\tCol3",\n  "suggested_tags": [],\n  "content_type": "note"\n}';

            const result = parseStructuredResponse(response);

            expect(result).not.toBeNull();
            expect(result?.body_content).toContain('Col1');
        });

        it('should handle the exact pattern LLMs produce: multi-line body_content with markdown', () => {
            // This is what the user reported: LLM returns JSON where body_content
            // contains full markdown with literal newlines
            const response = `{
  "summary_hook": "A writer replaced morning scrolling with Gemini",
  "body_content": "## Replace Your Morning Scroll

### The Lede
*One typed word triggers a briefing.*

### Why It Matters
* **Decision fatigue** is worst first thing in the morning.

### The Bottom Line
*Save yourself the scroll.*",
  "suggested_tags": ["productivity", "ai"],
  "content_type": "reference",
  "detected_language": "en"
}`;
            ...
            expect(result).not.toBeNull();
            expect(result?.summary_hook).toBe('A writer replaced morning scrolling with Gemini');
            expect(result?.body_content).toContain('### The Lede');
            expect(result?.body_content).toContain('### The Bottom Line');
            expect(result?.body_content).not.toContain('"summary_hook"');
            expect(result?.content_type).toBe('reference');
        });

        it('should parse fenced JSON with literal newlines and preamble text', () => {
            // LLM wraps malformed JSON in preamble + code fence
            // Fence extraction gets the inner JSON, but it has literal newlines → parse fails
            // Repair must fix the newlines, then fence/object re-extraction must succeed
            const response = `Here is the structured summary:

\`\`\`json
{
  "summary_hook": "Fenced with preamble",
  "body_content": "## Heading
...
\`\`\``;
            ...
            expect(result).not.toBeNull();
            expect(result?.summary_hook).toBe('Fenced with preamble');
            expect(result?.body_content).toContain('## Heading');
            expect(result?.body_content).toContain('multiple lines.');
            // Must not fall back to raw text (the original bug)
            expect(result?.body_content).not.toContain('"summary_hook"');
            expect(result?.body_content).not.toContain('```json');
        });

        it('should parse preamble-wrapped JSON object with literal newlines', () => {
            // LLM outputs bare JSON object with preamble (no fences)
            // Object extraction gets the {...}, but literal newlines break parse
            // Repair + object re-extraction must succeed
            const response = `I'll provide the summary as requested:

{
  "summary_hook": "Preamble no fence",
...
Line three",
  ...
  "content_type": "note"
            ...
            expect(result).not.toBeNull();
            expect(result?.summary_hook).toBe('Preamble no fence');
            expect(result?.body_content).toContain('Line one');
            expect(result?.body_content).toContain('Line three');
            expect(result?.body_content).not.toContain('"summary_hook"');
        });

        it('should parse fenced JSON where body_content contains inner code fences with literal newlines', () => {
            // Claude returns fenced JSON and body_content contains code examples
            // The non-greedy fence regex could match to the INNER code fence's closing ```
            const response = `\`\`\`json
{
  "summary_hook": "About Claude's building blocks",
  "body_content": "## Building Blocks

Skills are defined in folders:
...
Prompts are instant context.",
  "suggested_tags": ["claude", "skills"],
  "content_type": "reference"
}
\`\`\``;
            ...
            expect(result).not.toBeNull();
            expect(result?.summary_hook).toBe("About Claude's building blocks");
            expect(result?.body_content).toContain('## Building Blocks');
            expect(result?.body_content).toContain('Prompts are instant context.');
            expect(result?.body_content).not.toContain('"summary_hook"');
            expect(result?.body_content).not.toContain('```json');
        });

        it('should parse fenced JSON where body_content has escaped newlines (valid JSON)', () => {
            // Claude returns perfectly valid JSON with \n escape sequences in strings
            // This must parse on first or second attempt, never fall to fallback
            const response = '```json\n{\n  "summary_hook": "Claude has 5 building blocks.",\n  "body_content": "## Building Blocks\\n\\nSkills are folders.\\n\\n```python\\nskill = load()\\n```\\n\\nPrompts are context.",\n  "suggested_tags": ["claude"],\n  "content_type": "reference"\n}\n```';

            const result = parseStructuredResponse(response);

            expect(result).not.toBeNull();
            expect(result?.summary_hook).toBe('Claude has 5 building blocks.');
            expect(result?.body_content).toContain('## Building Blocks');
            expect(result?.body_content).toContain('Prompts are context.');
            expect(result?.body_content).not.toContain('"summary_hook"');
        });

        it('should parse fenced JSON with body_content containing quotes and markdown', () => {
            // body_content has markdown with bold, inline code, and em-dash
            const response = `\`\`\`json
{
  "summary_hook": "A guide to skills and prompts.",
  "body_content": "## Claude\u2019s Agentic Building Blocks \u2014 A Practical Guide

Anthropics guide breaks down the five core components of Claude\u2019s agentic ecosystem. The five building blocks are **Skills**, **Prompts**, **Projects**, **Subagents**, and **MCP (Model Context Protocol)**.

### What Each Building Block Does

- **Skills** \u2013 Folders of instructions, scripts, and resources that load dynamically using *progressive disclosure* (metadata first \u2192 full instructions \u2192 bundled assets). Best for reusable procedural knowledge.
- **Prompts** \u2013 Natural language instructions given in the moment. Ephemeral and reactive; ideal for one-off requests.",
  "suggested_tags": ["claude", "ai-agents", "skills", "mcp"],
  "content_type": "reference",
  "detected_language": "en"
}
\`\`\``;
            ...
            expect(result).not.toBeNull();
            expect(result?.summary_hook).toBe('A guide to skills and prompts.');
            expect(result?.body_content).toContain('### What Each Building Block Does');
            expect(result?.body_content).toContain('**Skills**');
            expect(result?.body_content).not.toContain('"summary_hook"');
            expect(result?.body_content).not.toContain('```json');
        });
    });

    describe('Tier 5: Fallback to Plain Text', () => {
...
describe('Response Parser - Generic JSON Extraction', () => {
    ...
    describe('tryExtractJson', () => {
        ...
        it('should repair literal newlines in JSON strings and parse successfully', () => {
            // JSON with literal newlines inside a string value — invalid for JSON.parse
            const text = '{\n  "actions": [{"id": "A1", "text": "Review the draft\nand send to Bob"}]\n}';
            const result = tryExtractJson(text) as any;

            expect(result).not.toBeNull();
            expect(result.actions[0].text).toBe('Review the draft\nand send to Bob');
        });

        it('should repair literal newlines in code-fenced JSON', () => {
            const text = 'Here:\n```json\n{"key": "line1\nline2"}\n```';
            const result = tryExtractJson(text) as any;
...
describe('Response Parser - repairJsonStrings', () => {
    it('should return identical string when no repair needed', () => {
        const valid = '{"key": "value"}';
        expect(repairJsonStrings(valid)).toBe(valid);
    });

    it('should escape literal newlines inside JSON strings', () => {
        const broken = '{"text": "line one\nline two"}';
        const repaired = repairJsonStrings(broken);
        expect(repaired).toBe('{"text": "line one\\nline two"}');
        expect(JSON.parse(repaired)).toEqual({ text: 'line one\nline two' });
    });

    it('should escape literal tabs inside JSON strings', () => {
        const broken = '{"text": "col1\tcol2"}';
        const repaired = repairJsonStrings(broken);
        expect(repaired).toBe('{"text": "col1\\tcol2"}');
        expect(JSON.parse(repaired)).toEqual({ text: 'col1\tcol2' });
    });

    it('should escape literal carriage returns inside JSON strings', () => {
        const broken = '{"text": "line one\r\nline two"}';
        const repaired = repairJsonStrings(broken);
        expect(JSON.parse(repaired)).toEqual({ text: 'line one\r\nline two' });
    });

    it('should NOT escape newlines outside strings (structural whitespace)', () => {
        const valid = '{\n  "key": "value"\n}';
        const repaired = repairJsonStrings(valid);
        // Structural newlines between fields should be preserved as-is
        expect(repaired).toContain('\n');
        expect(JSON.parse(repaired)).toEqual({ key: 'value' });
    });

    it('should handle already-escaped sequences (no double-escaping)', () => {
        const valid = String.raw`{"text": "line one\nline two"}`;
        expect(repairJsonStrings(valid)).toBe(valid);
    });

    it('should handle escaped quotes inside strings', () => {
        const input = String.raw`{"text": "say \"hello\""}`;
        const repaired = repairJsonStrings(input);
        expect(repaired).toBe(input); // Nothing to repair
    });

    it('should handle multi-field JSON with newlines in one field', () => {
        const broken = '{\n  "hook": "short",\n  "body": "## Title\n\nContent here"\n}';
        const repaired = repairJsonStrings(broken);
        const parsed = JSON.parse(repaired);
        expect(parsed.hook).toBe('short');
        expect(parsed.body).toBe('## Title\n\nContent here');
    });
});

describe('Response Parser - Greedy Fence & Field Extraction', () => {
    it('should use greedy fence when non-greedy fails due to inner code fences (valid JSON)', () => {
        // Valid JSON with properly escaped inner code fences — greedy fence should match
        const response = '```json\n{\n  "summary_hook": "Hook",\n  "body_content": "## Title\\n\\n```python\\ncode\\n```\\n\\nMore text",\n  "suggested_tags": ["test"],\n  "content_type": "note"\n}\n```';

        const result = parseStructuredResponse(response);

        expect(result).not.toBeNull();
        expect(result?.summary_hook).toBe('Hook');
        expect(result?.body_content).toContain('## Title');
        expect(result?.body_content).toContain('More text');
        expect(result?.body_content).not.toContain('"summary_hook"');
    });

    it('should handle \\r\\n line endings in code fence', () => {
        const response = '```json\r\n{\r\n  "summary_hook": "Hook",\r\n  "body_content": "Content here",\r\n  "suggested_tags": [],\r\n  "content_type": "note"\r\n}\r\n```';
    ...
    it('should extract fields when JSON.parse entirely fails', () => {
        // Simulate completely broken JSON that still has identifiable fields
        // Body content has unescaped quotes that break JSON.parse
        const response = `\`\`\`json
{
  "summary_hook": "A guide to tools",
  "body_content": "## Tools

Use the "advanced" mode for better results.

The system says "hello world" on startup.",
  "suggested_tags": ["tools", "guide"],
  "content_type": "reference"
}
\`\`\``;
        ...
        expect(result).not.toBeNull();
        // The field extraction should get summary_hook correctly
        expect(result?.summary_hook).toBe('A guide to tools');
        // body_content should contain the markdown, not raw JSON
        expect(result?.body_content).toContain('## Tools');
        expect(result?.body_content).not.toContain('"summary_hook"');
    });

    it('should extract body_content with inner triple backticks and literal newlines via field extraction', () => {
        // This simulates the exact failure mode: body_content has inner code fences
        // AND literal newlines, AND quotes that break repairJsonStrings
        const response = `\`\`\`json
{
  "summary_hook": "Building blocks explained",
  "body_content": "## Building Blocks
...
Each building block has a "purpose" and "scope".",
  "suggested_tags": ["claude"],
  "content_type": "reference"
}
\`\`\``;
        ...
        expect(result).not.toBeNull();
        expect(result?.summary_hook).toBe('Building blocks explained');
        expect(result?.body_content).toContain('## Building Blocks');
        expect(result?.body_content).not.toContain('"summary_hook"');
    });

    it('should not detect plain markdown as raw JSON', () => {
        // Plain markdown should NOT trigger looksLikeRawJson
        const markdown = '## Title\n\nSome content about building blocks.\n\n- Point one\n- Point two';

        // If parseStructuredResponse gets this, it should fall to fallback
        // But the fallback body_content should not be detected as raw JSON by summarizeAndInsert
        const result = parseStructuredResponse(markdown);
        expect(result).not.toBeNull();
        expect(result?.body_content).toBe(markdown); // Fallback sets raw text
        // The body_content does NOT contain JSON keys, so looksLikeRawJson would be false
        expect(result?.body_content).not.toContain('"body_content"');
    });
});
