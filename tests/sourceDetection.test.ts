...
describe('Source Detection - YouTube Patterns', () => {
    ...
    describe('isYouTubeUrl helper', () => {
        ...
        it('should handle standard youtube.com casing', () => {
...
describe('Source Detection - Mixed Content', () => {
...
describe('Source Detection - Image Patterns', () => {

    describe('Vault Image Link Detection', () => {
        it('should detect ![[photo.png]] as image source', () => {
            const content = 'Here is a photo: ![[photo.png]]';
            const sources = detectSourcesFromContent(content);

            expect(sources.images.length).toBe(1);
            expect(sources.images[0].type).toBe('image');
            expect(sources.images[0].isVaultFile).toBe(true);
            expect(sources.images[0].value).toBe('photo.png');
        });

        it('should detect [[diagram.jpg]] without embed prefix', () => {
            const content = '[[diagram.jpg]]';
            const sources = detectSourcesFromContent(content);

            expect(sources.images.length).toBe(1);
            expect(sources.images[0].value).toBe('diagram.jpg');
        });

        it('should detect images with subdirectory paths', () => {
            const content = '![[Attachments/photos/vacation.jpeg]]';
            const sources = detectSourcesFromContent(content);

            expect(sources.images.length).toBe(1);
            expect(sources.images[0].value).toBe('Attachments/photos/vacation.jpeg');
        });
...
![[sketch.bmp]]`;
            ...
            expect(sources.images.length).toBe(4);
        });

        it('should handle spaces in image paths', () => {
            const content = '![[My Photos/family photo.png]]';
            const sources = detectSourcesFromContent(content);

            expect(sources.images.length).toBe(1);
            expect(sources.images[0].value).toBe('My Photos/family photo.png');
        });
        ...
        it('should detect image with Obsidian size suffix ![[photo.png|300]]', () => {
            const content = '![[photo.png|300]]';
            const sources = detectSourcesFromContent(content);
        ...
        it('should detect image with Obsidian alias ![[photo.png|My Photo]]', () => {
            const content = '![[diagram.svg|Architecture Diagram]]';
            const sources = detectSourcesFromContent(content);

            expect(sources.images.length).toBe(1);
            expect(sources.images[0].value).toBe('diagram.svg');
        });

        it('should detect image with size suffix and path', () => {
            const content = '![[Attachments/photo.jpg|400x300]]';
            const sources = detectSourcesFromContent(content);

            expect(sources.images.length).toBe(1);
            expect(sources.images[0].value).toBe('Attachments/photo.jpg');
        });
    });

    describe('Image NOT detected as URL', () => {
        it('should NOT detect external image URLs (v1 scope)', () => {
            const content = 'External: https://example.com/photo.png';
            const sources = detectSourcesFromContent(content);

            // External image URL goes to urls, NOT images
            expect(sources.images.length).toBe(0);
            expect(sources.urls.length).toBe(1);
        });

        it('should NOT double-count vault images as other types', () => {
            const content = '![[photo.png]]';
            const sources = detectSourcesFromContent(content);

            expect(sources.images.length).toBe(1);
            expect(sources.urls.length).toBe(0);
            expect(sources.pdfs.length).toBe(0);
            expect(sources.documents.length).toBe(0);
        });
    });

    describe('getTotalSourceCount includes images', () => {
        it('should count images in total', () => {
            const content = `![[photo.png]]
https://example.com`;
            const sources = detectSourcesFromContent(content);
            ...
            expect(sources.images.length).toBe(1);
            expect(sources.urls.length).toBe(1);
            expect(getTotalSourceCount(sources)).toBe(2);
        });
    });

    describe('removeProcessedSources does NOT remove images', () => {
        it('should preserve image embeds when removing other vault files', () => {
            const content = `![[photo.png]]
![[meeting.pdf]]
Some notes`;
            // Only pass meeting.pdf as vault file to remove, not the image
            const result = removeProcessedSources(content, [], ['meeting.pdf']);
