...
describe('Structured Prompts - Invariants', () => {
    describe('buildStructuredSummaryPrompt', () => {
        ...
        it('should use Smart Brevity format for brief length', () => {
            const prompt = buildStructuredSummaryPrompt({ length: 'brief' });
...
describe('Translate Prompts - Invariants', () => {
    ...
    describe('buildTranslatePrompt - Chunk Position Labels', () => {
        it('should inject chunk context when chunkIndex and chunkTotal set', () => {
        ...
        it('should NOT inject chunk context for single chunk', () => {
            const prompt = buildTranslatePrompt({
                targetLanguage: 'French',
                chunkIndex: 1,
                chunkTotal: 1,
            ...
            expect(prompt).not.toContain('consistent terminology');
        });

        it('should NOT inject chunk context when metadata absent', () => {
            const prompt = buildTranslatePrompt({ targetLanguage: 'French' });
            expect(prompt).not.toContain('consistent terminology');
        });

        it('should combine source context with chunk context', () => {
            const prompt = buildTranslatePrompt({
