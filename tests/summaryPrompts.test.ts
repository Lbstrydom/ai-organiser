...
describe('buildSummaryPrompt', () => {
  describe('basic prompt generation', () => {
    ...
    it('should include link handling instructions', () => {
      ...
      expect(prompt).toContain('PRESERVE important markdown links');
    });
  });

  describe('length options', () => {
    it('should include Smart Brevity instructions for brief length', () => {
      const options: SummaryPromptOptions = {
        length: 'brief',
      };
      const prompt = buildSummaryPrompt(options);
...
describe('chunk metadata in prompts', () => {
  it('should inject chunk context block when chunkIndex and chunkTotal set', () => {
    const options: SummaryPromptOptions = {
      length: 'detailed',
      chunkIndex: 2,
      chunkTotal: 5,
    ...
    expect(prompt).toContain('Do not write an introduction or conclusion');
  });

  it('should inject chunk context block with persona prompt', () => {
    const options: SummaryPromptOptions = {
      length: 'detailed',
      personaPrompt: 'Research analyst persona',
      chunkIndex: 1,
      chunkTotal: 3,
    };
    const prompt = buildSummaryPrompt(options);
    ...
    expect(prompt).toContain('Research analyst persona');
  });

  it('should NOT inject chunk context block for single-chunk (chunkTotal=1)', () => {
    const options: SummaryPromptOptions = {
      length: 'standard',
      chunkIndex: 1,
      chunkTotal: 1,
  ...
  it('should NOT inject chunk context block when chunkIndex not set', () => {
    const options: SummaryPromptOptions = {
      length: 'standard',
    };
    const prompt = buildSummaryPrompt(options);
