...
- **2026-06-05 (cont.)** — Track-1 UI flows finished: **multi-source → PASS** (the harness bug was `dismissSecondary` matching the multi-source modal and pre-clicking its own "Summarize" CTA — fixed by scoping the exclude-selector; consent appears only *after* Summarize and is dismissed during the poll). **Minutes**: the CTA-gate **product bug is fixed + verified**; full E2E completion is **not cleanly automatable in this vault** because the modal scans `AI-Organiser/Recordings/`, auto-loads the detected audio, and **transcribes all of it on Generate** — a heavy multi-audio path that stalls under the 10k TPM (QUOTA, not a defect). Minutes' Azure egress (`summarizeText`) is covered by the service sweep; the gate + generation-start are verified. **UX observation to review**: opening Minutes auto-loads + (on Generate) transcribes every recording in the Recordings folder even when the user typed their own transcript — intended convenience, or surprising/costly? Worth a deliberate decision.
- Remaining TODO: wire per-persona deck generation into the layout radar → first `--bless` of a clean baseline. (Make the radar's vision-judge model configurable — gpt-4o / Azure-Opus / Gemini — per the vision-vs-deterministic discussion: geometry stays the hard gate for precise defects; the configurable vision model is the soft holistic layer.)
