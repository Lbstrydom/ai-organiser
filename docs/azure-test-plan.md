...
- **2026-06-05 (cont.)** — **Full persona-fit run (Pat + Chen + Maya), live Azure-Opus judge.** Each generated a real 8-slide deck via the presentation UI + was judged (geometry + Opus vision):
  ...
  - **Real product finding (presentation charts) — FIXED:** bar charts with **long category labels** wrapped the Y-axis label to two lines and **crowded/overlapped the bars** (label column fixed at HTML `200px` / PPTX `1.2"`). Surfaced by the Opus vision layer on BOTH Chen + Maya. Fix (`irToHtml.ts` + `irToPptx.ts`, in parity): widened the label column to **280px / 1.9"** + `line-height:1.25`. Verified: re-generated Maya's deck → **LAYOUT-OK** (chart slide passes; even longer labels still wrap but now sit clear of the bars — the overlap is gone).
