...
- Remaining TODO: minutes-completion-poll tuning + multi-source consent flow (egress already covered by the sweep) → wire per-persona deck generation into the radar → first `--bless` of a clean baseline.
