...
- **2026-06-05 (cont.)** — `azure-ui-flows.mjs` (poll-on-state) live; **transcribe (Whisper) → PASS** in Azure mode: drives the real `TranscribeOnlyModal` → `DocumentMultiPickerModal` (audio-scoped) → Whisper transcribe + Claude speaker-labelling → "Transcription complete", `azureRouted=true`, 0 hard errors (1 `[rate-limit azure-whisper]` observed + handled). Note: the vault pick is the **scoped multi-picker** (in-note-first), not a fuzzy modal — not a bug. Remaining TODO: minutes / multi-source / presentation UI flows → Track-2 persona-layout A/B → Track-3 bless.
