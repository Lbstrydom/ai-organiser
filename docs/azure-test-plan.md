...
- **2026-06-05 (cont.)** — **Full persona-fit run (Pat + Chen + Maya), live Azure-Opus judge.** Each generated a real 8-slide deck via the presentation UI + was judged (geometry + Opus vision):
  ...
  - **Real product finding (presentation charts):** bar charts with **long category labels** ("Short-Term Memory", "Transformer (Big)") wrap the Y-axis label to two lines and **crowd/overlap the bars** — the label column is too narrow. Surfaced by the Opus vision layer on BOTH Chen + Maya. Candidate fix: widen the chart label column or truncate/abbreviate long labels. (Bars themselves scale correctly — minor but real.)
- Remaining TODO: fix the chart long-label overlap → re-run → `--bless` the first clean baseline (Track 3, per "fix before bless").
