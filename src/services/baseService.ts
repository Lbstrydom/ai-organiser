...
export abstract class BaseLLMService {
     ...
     * @param seconds - Timeout in seconds (minimum 30, maximum 900)
     */
    public setSummarizeTimeout(seconds: number): void {
        // Clamp between 30 seconds and 15 minutes
        const clampedSeconds = Math.max(30, Math.min(900, seconds));
    ...
    protected parseTagsResponse(response: string): { tags: string[]; suggestedTitle?: string; suggestedFolder?: string } {
        ...
        try {
            ...
            if (jsonStr.startsWith('{')) {
                ...
                if (result.tags.length === 0 && (parsed.theme || parsed.discipline || parsed.topics)) {
                    const tierTags: string[] = [];
