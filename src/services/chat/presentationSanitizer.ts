...
// ── SVG markup sanitization (structured-IR `svg` block — plan H1) ────────────
 ...
 * URLs; plan, Gemini-r2 M1). Strips `<script>`/`<foreignObject>`/`<use>`/
 * external refs, every `on*` handler, and `href`/`xlink:href`.
 *
