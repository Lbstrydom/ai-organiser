...
export interface AudioTranscriptionConfig {
    provider: 'openai' | 'groq' | 'azure' | 'azure-speech' | 'openai-gpt-audio';
    apiKey: string;
    language?: string;
...
export class ContentExtractionService {
    ...
    constructor(
        ...
        youtubeGeminiConfig?: Omit<YouTubeGeminiConfig, 'url'>
    ) {
        this.app = app;
    ...
    private async extractYouTubeContent(item: DetectedContent): Promise<ExtractedContent> {
        try {
            ...
            if (this.youtubeGeminiConfig?.apiKey) {
                logger.debug('Core', 'Using Gemini for YouTube transcription');
    ...
    private async extractPdfWithMultimodal(item: DetectedContent): Promise<ExtractedContent> {
        try {
            ...
            if (!pdfResult || !pdfResult.success || !pdfResult.content) {
                // Fall back to text extraction
                logger.debug('Core', 'PDF base64 extraction failed, falling back to text extraction');
                return this.extractPdfAsText(item);
            }
