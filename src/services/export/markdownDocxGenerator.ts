...
export async function generateDocx(
    ...
    if (options.includeTitle && options.title) {
        children.push(new Paragraph({
            children: [new TextRun({ text: options.title, bold: true, size: 36, font: 'Calibri' })],
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (tableEntry) {
            children.push(buildDocxTable(tableEntry.table, { Table, TableRow, TableCell, Paragraph, TextRun, WidthType, BorderStyle, ShadingType }));
        ...
        switch (line.type) {
            case 'heading1':
                children.push(new Paragraph({
                    children: [new TextRun({ text: line.content, bold: true, size: 32, font: 'Calibri' })],
            ...
            case 'heading2':
                children.push(new Paragraph({
                    children: [new TextRun({ text: line.content, bold: true, size: 26, font: 'Calibri' })],
            ...
            case 'heading3':
                children.push(new Paragraph({
                    children: [new TextRun({ text: line.content, bold: true, size: 24, font: 'Calibri' })],
            ...
            case 'bullet':
                children.push(new Paragraph({
                    children: [new TextRun({ text: line.content, size: 22, font: 'Calibri' })],
    ...
    const headerRow = new TableRow({
        ...
        children: table.headers.map((header: string) => new TableCell({
            children: [new Paragraph({
                children: [new TextRun({ text: header, bold: true, size: 20, font: 'Calibri' })],
            })],
