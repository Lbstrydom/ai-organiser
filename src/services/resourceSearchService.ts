    ...
    if (wantsYouTube || !wantsArticles) {
        for (const term of searchTerms.slice(0, 2)) {
            try {
                ...
                console.warn('[AI Organiser] YouTube search failed for term:', term, error);
            }
        }
    }
    ...
    if (wantsArticles || !wantsYouTube) {
        for (const term of searchTerms.slice(0, 2)) {
            try {
                ...
                console.warn('[AI Organiser] DuckDuckGo search failed for term:', term, error);
            }
        }
    }
    ...
    try {
        ...
        if (dataMatch) {
            try {
                ...
                console.warn('[AI Organiser] Failed to parse YouTube data:', parseError);
            }
        }
        ...
        console.warn('[AI Organiser] YouTube search error:', error);
    }
...
async function searchDuckDuckGo(query: string): Promise<ResourceSearchResult[]> {
    ...
    try {
        ...
        console.warn('[AI Organiser] DuckDuckGo search error:', error);
    }
