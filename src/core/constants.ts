 ...
 * Valid source types
 */
export const SOURCE_TYPES = ['url', 'pdf', 'youtube', 'audio', 'note', 'kindle', 'email'] as const;
...
export const IMAGE_CONVERSION_REQUIRED: ReadonlyArray<string> = [
    '.heic', '.heif', '.bmp', '.tiff', '.tif', '.avif', '.svg'
];
export const VLM_NATIVE_IMAGE_FORMATS: ReadonlyArray<string> = [
    'image/jpeg', 'image/png', 'image/webp', 'image/gif'
];

// Image processing defaults
export const DEFAULT_IMAGE_MAX_DIMENSION = 1536;  // Optimal for all VLM providers
export const DEFAULT_IMAGE_QUALITY = 0.85;         // JPEG quality
export const MEDIA_SIZE_WARN_BYTES = 5 * 1024 * 1024;  // 5MB threshold for compression UI
...
// Minutes style — replaces persona + detail level (Phase 2, TRA plan)
export type MinutesStyle = 'smart-brevity' | 'standard' | 'detailed' | 'guided';
export const DEFAULT_MINUTES_STYLE: MinutesStyle = 'standard';
...
// === KINDLE PHASE 3: AMAZON CLOUD SYNC ===

/** Amazon region configurations for Kindle sync */
export const AMAZON_REGIONS: Record<string, { label: string; domain: string }> = {
    'com':     { label: 'United States', domain: 'com' },
    'co.uk':   { label: 'United Kingdom', domain: 'co.uk' },
    'ca':      { label: 'Canada', domain: 'ca' },
    'com.au':  { label: 'Australia', domain: 'com.au' },
    'de':      { label: 'Germany', domain: 'de' },
    'fr':      { label: 'France', domain: 'fr' },
    'es':      { label: 'Spain', domain: 'es' },
    'it':      { label: 'Italy', domain: 'it' },
    'co.jp':   { label: 'Japan', domain: 'co.jp' },
    'com.br':  { label: 'Brazil', domain: 'com.br' },
    'com.mx':  { label: 'Mexico', domain: 'com.mx' },
    'nl':      { label: 'Netherlands', domain: 'nl' },
    'in':      { label: 'India', domain: 'in' },
};
