...
export class ResourceResultsModal extends Modal {
    ...
    onOpen() {
        ...
        contentEl.empty();
        contentEl.addClass('resource-results-modal');
        ...
        contentEl.createEl('p', {
            ...
            cls: 'resource-results-description'
        });
        ...
        // Instructions
        const instructions = contentEl.createEl('div', { cls: 'resource-results-instructions' });
        ...
        // Close button
        const buttonContainer = contentEl.createEl('div', { cls: 'resource-results-buttons' });
    ...
    private renderSection(
        ...
        const section = container.createEl('div', { cls: 'resource-results-section' });

        const header = section.createEl('div', { cls: 'resource-results-section-header' });
        const iconEl = header.createEl('span', { cls: 'resource-results-section-icon' });
        setIcon(iconEl, icon);
        header.createEl('span', { text: title });

        const list = section.createEl('div', { cls: 'resource-results-list' });

        for (const result of results) {
            const item = list.createEl('div', { cls: 'resource-results-item' });

            // Thumbnail for YouTube
            if (result.source === 'youtube' && result.thumbnail) {
                const thumbContainer = item.createEl('div', { cls: 'resource-results-thumb' });
            ...
            const info = item.createEl('div', { cls: 'resource-results-info' });

            // Title as link
            const titleLink = info.createEl('a', {
                text: result.title,
                cls: 'resource-results-title',
                attr: {
                    href: result.url,
                    target: '_blank',
            ...
            if (result.description) {
                info.createEl('p', {
                    ...
                    cls: 'resource-results-desc'
                });
            }

            // URL and copy button
            const urlRow = info.createEl('div', { cls: 'resource-results-url-row' });
            urlRow.createEl('span', {
                text: this.truncateUrl(result.url),
                cls: 'resource-results-url'
            });

            const copyButton = urlRow.createEl('button', {
                cls: 'resource-results-copy',
