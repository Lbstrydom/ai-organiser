...
export class RelatedNotesModal extends Modal {
    ...
    async onOpen(): Promise<void> {
        ...
        contentEl.empty();
        contentEl.addClass('ai-organiser-related-notes-modal');

        // Derive initial folder scope from active file
        const currentFile = this.app.workspace.getActiveFile();
        this.folderScope = this.normalizeFolderPath(currentFile?.parent?.path);

        const header = contentEl.createDiv({ cls: 'ai-organiser-related-notes-header' });
        header.createEl('h3', { text: this.plugin.t.commands.showRelatedNotes });

        // Scope toggle + refresh in a controls row
        const controls = header.createDiv({ cls: 'ai-organiser-related-notes-modal-controls' });

        const t = this.plugin.t?.modals?.relatedNotes;

        // Scope toggle buttons
        const scopeGroup = controls.createDiv({ cls: 'ai-organiser-related-notes-scope-toggle' });
    ...
    private async fetchRelatedNotes(): Promise<void> {
        ...
        try {
            ...
            logger.error('Search', 'Related notes modal error:', error);
    ...
    private renderResults(): void {
        ...
        for (const result of this.results) {
            const itemEl = this.listEl.createEl('li', { cls: 'ai-organiser-related-notes-item' });
