...
export class PdfSelectModal extends Modal {
    ...
    onOpen(): void {
        ...
        contentEl.addClass('ai-organiser-pdf-select-modal');
        ...
        const listEl = contentEl.createEl('div', { cls: 'ai-organiser-pdf-list' });

        // Show max 15 recent PDFs
        this.files.slice(0, 15).forEach(file => {
            const item = listEl.createEl('div', { cls: 'ai-organiser-pdf-list-item' });

            const infoDiv = item.createEl('div', { cls: 'ai-organiser-pdf-info' });
            infoDiv.createEl('span', { text: file.name, cls: 'ai-organiser-pdf-name' });
