...
/**
 * Latest and most popular local LLM models (2026)
 */
const LOCAL_MODEL_RECOMMENDATIONS: ModelRecommendation[] = [
    // Chat models - sorted by quality/popularity
    {
        ...
        quality: 'excellent',
        speed: 'slow',
        description: 'Best open-source model. Near GPT-4 quality. Requires high-end hardware.'
    },
    {
        name: 'Qwen 2.5 32B',
        ...
        quality: 'excellent',
        speed: 'medium',
        description: 'Excellent multilingual model. Great for coding and reasoning.'
    },
    {
    ...
    {
        name: 'Llama 3.2 11B',
        ollamaId: 'llama3.2:11b',
        sizeGB: 6.5,
        minRamGB: 12,
        type: 'chat',
        quality: 'good',
        speed: 'medium',
        description: 'Great balance of speed and quality. Good for most tasks.'
    },
    {
    ...
    },
    {
        ...
        quality: 'good',
        speed: 'medium',
        description: 'Google\'s latest. Strong instruction following.'
    },
    {
        name: 'Phi-4 14B',
        ollamaId: 'phi4:14b',
        ...
        quality: 'good',
        speed: 'medium',
        description: 'Microsoft\'s latest. Excellent for reasoning and math.'
    },
    {
        name: 'Qwen 2.5 7B',
        ollamaId: 'qwen2.5:7b',
        ...
        quality: 'good',
        speed: 'fast',
        description: 'Compact but capable. Good multilingual support.'
    },
    {
        name: 'Llama 3.2 3B',
    ...
    },
    {
        ...
        quality: 'basic',
        speed: 'fast',
        description: 'Very small footprint. Fast responses.'
    },
...
export class LocalSetupWizardModal extends Modal {
    ...
    async onOpen() {
        ...
        contentEl.empty();
        contentEl.addClass('local-setup-wizard');
    ...
    private renderProgressIndicator(container: HTMLElement): void {
        const progressEl = container.createDiv('wizard-progress');
    ...
    private renderStep1(container: HTMLElement): void {
        const stepEl = container.createDiv('wizard-step');
        ...
        if (this.status.ollama.running) {
            statusEl.innerHTML = `<span class="status-success">✓</span> Ollama is running${this.status.ollama.version ? ` (v${this.status.ollama.version})` : ''}`;
        } else {
            statusEl.innerHTML = '<span class="status-warning">⚠</span> Ollama not detected';
        }
    ...
    private renderModelSection(container: HTMLElement, title: string, type: 'chat' | 'embedding' | 'multimodal'): void {
        const sectionEl = container.createDiv('model-section');
        ...
        for (const model of models) {
            ...
            const modelEl = sectionEl.createDiv({
                cls: `model-item ${!isCompatible ? 'incompatible' : ''} ${isRecommended ? 'recommended' : ''} ${isInstalled ? 'installed' : ''}`
            });

            const headerEl = modelEl.createDiv('model-header');
            headerEl.createEl('strong', { text: model.name });

            const badges = headerEl.createDiv('model-badges');
            ...
            modelEl.createEl('p', { cls: 'model-description', text: model.description });

            const infoEl = modelEl.createDiv('model-info');
            infoEl.createSpan({ text: `Size: ${model.sizeGB}GB` });
            infoEl.createSpan({ text: `RAM: ${model.minRamGB}GB+` });

            // Install/Select button
            const actionsEl = modelEl.createDiv('model-actions');
    ...
    private async installModel(modelId: string, button: HTMLButtonElement): Promise<void> {
        ...
        new Notice(`Command copied: ${command}\n\nRun this in your terminal to install the model.`, 10000);
    ...
    private async applySettings(): Promise<void> {
        ...
        new Notice('Local AI settings applied successfully!');
    }
    ...
    private renderNavigation(container: HTMLElement, showBack: boolean, showNext: boolean): void {
        const navEl = container.createDiv('wizard-navigation');
