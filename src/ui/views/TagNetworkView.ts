...
export class TagNetworkView extends ItemView {
    ...
    private render(): void {
        ...
        contentEl.empty();
        contentEl.addClass('ai-organiser-tag-network-view');
        ...
        const container = contentEl.createDiv({ cls: 'ai-organiser-tag-network-container' });
        const tooltip = contentEl.createDiv({ cls: 'tag-tooltip tag-tooltip-hidden' });
        tooltip.createDiv({ cls: 'tag-tooltip-content' });

        const statusEl = contentEl.createDiv({ cls: 'ai-organiser-tag-network-status' });
    ...
    private buildTagSearchInput(
        ...
        const t = this.plugin.t.tagNetwork;
        const controlsContainer = parent.createDiv({ cls: 'ai-organiser-tag-network-controls' });
        const searchContainer = controlsContainer.createDiv({ cls: 'ai-organiser-tag-network-search' });
        const chipContainer = searchContainer.createDiv({ cls: 'ai-organiser-tag-network-chip-container' });
        const input = chipContainer.createEl('input', {
            type: 'text',
            placeholder: t.searchPlaceholder,
            cls: 'ai-organiser-tag-network-search-input'
        });
        ...
        const renderDropdown = (activeIndex: number) => {
            ...
            suggestions.forEach((suggestion, index) => {
                const item = dropdown.createDiv({
                    cls: `ai-organiser-tag-network-dropdown-item${index === activeIndex ? ' active' : ''}`
                });
                item.setAttr('data-id', suggestion.id);
                item.createDiv({ text: suggestion.label });
                item.createDiv({ cls: 'ai-organiser-tag-network-dropdown-freq', text: String(suggestion.frequency) });
        ...
        const addChip = (id: string) => {
            ...
            const chip = chipContainer.createDiv({ cls: 'ai-organiser-tag-network-chip' });
            chip.dataset.chipId = id;
            chip.createDiv({ cls: 'ai-organiser-tag-network-chip-label', text: node.label });
            const removeButton = chip.createEl('button', { cls: 'ai-organiser-tag-network-chip-remove', text: '×' });
    ...
    private buildLegend(parent: HTMLElement): void {
        const t = this.plugin.t.tagNetwork;
        const legendContainer = parent.createDiv({ cls: 'ai-organiser-tag-network-legend' });
        ...
        for (const itemData of legendItems) {
            const item = legendContainer.createDiv({ cls: 'ai-organiser-tag-network-legend-item' });
            item.createDiv({ cls: `ai-organiser-tag-network-legend-color ${itemData.key}` });
    ...
    private renderD3Network(
        ...
        const link = g.append('g')
            .attr('class', 'ai-organiser-tag-network-link')
            .selectAll('line')
            .data(links)
            .join('line')
        ...
        const node = g.append('g')
            ...
            .data(nodes)
            .join('circle')
            .attr('class', 'ai-organiser-tag-network-node')
        ...
        const labels = g.append('g')
            ...
            .data(nodes)
            .join('text')
            .attr('class', 'ai-organiser-tag-network-label')
    ...
    private renderMobileList(container: HTMLElement, searchState: TagSearchState, statusEl: HTMLElement): () => void {
        const t = this.plugin.t.tagNetwork;
        container.empty();
        const listEl = container.createEl('ul', { cls: 'ai-organiser-tag-network-list' });
        ...
        const renderList = () => {
            ...
            for (const n of filtered) {
                const itemEl = listEl.createEl('li', { cls: 'ai-organiser-tag-network-list-item' });
                ...
                itemEl.createEl('small', {
                    cls: 'ai-organiser-tag-network-list-meta',
