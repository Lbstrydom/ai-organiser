...
export class RelatedNotesView extends ItemView {
    ...
    private rerenderHeader(): void {
        ...
        // Remove old header
        const oldHeader = this.headerContainer.querySelector('.ai-organiser-related-notes-header');
    ...
    private renderHeader(container: HTMLElement): void {
        const t = this.plugin.t?.modals?.relatedNotes;
        const header = container.createEl('div', { cls: 'ai-organiser-related-notes-header' });
        ...
        const scopeBtn = leftGroup.createEl('button', {
            cls: 'clickable-icon ai-organiser-related-notes-scope-btn',
        ...
        if (this.state.folderScope !== null) {
            const pinBtn = leftGroup.createEl('button', {
                cls: `clickable-icon ai-organiser-related-notes-pin-btn${this.state.scopePinned ? ' is-pinned' : ''}`,
        ...
        const controls = header.createEl('div', { cls: 'ai-organiser-related-notes-controls' });
    ...
    private renderLoadingState(): void {
        ...
        this.resultContainer.createEl('div', {
            cls: 'ai-organiser-related-notes-loading',
    ...
    private renderEmptyState(message?: string): void {
        ...
        this.resultContainer.empty();
        const emptyEl = this.resultContainer.createEl('div', { cls: 'ai-organiser-related-notes-empty' });
        ...
        emptyEl.createEl('small', {
            ...
            cls: 'ai-organiser-related-notes-empty-hint'
        });
    }
    ...
    private renderScopedEmptyState(): void {
        ...
        this.resultContainer.empty();
        const t = this.plugin.t?.modals?.relatedNotes;
        const emptyEl = this.resultContainer.createEl('div', { cls: 'ai-organiser-related-notes-scoped-empty' });
    ...
    private renderDisabledState(reason: string): void {
        ...
        this.resultContainer.empty();
        const disabledEl = this.resultContainer.createEl('div', { cls: 'ai-organiser-related-notes-disabled' });
        ...
        disabledEl.createEl('small', {
            ...
            cls: 'ai-organiser-related-notes-disabled-hint'
        });
    }
    ...
    private renderErrorState(): void {
        ...
        this.resultContainer.empty();
        const errorEl = this.resultContainer.createEl('div', { cls: 'ai-organiser-related-notes-error' });
        errorEl.createEl('p', { text: t?.error || 'Error' });
        errorEl.createEl('small', {
            text: this.state.error || 'Unknown error',
            cls: 'ai-organiser-related-notes-error-message'
        });
    ...
    private renderResults(): void {
        ...
        const listEl = this.resultContainer.createEl('ul', { cls: 'ai-organiser-related-notes-list' });

        // Dismiss popup on scroll (prevents orphaned popups)
        listEl.addEventListener('scroll', () => this.hidePreviewPopup());

        for (const result of this.state.results) {
            const itemEl = listEl.createEl('li', { cls: 'ai-organiser-related-notes-item' });

            // Main link
            const linkEl = itemEl.createEl('a', {
                cls: 'ai-organiser-related-notes-link internal-link'
            });
            ...
            itemEl.createEl('span', {
                cls: `ai-organiser-related-notes-score ${scoreClass}`,
                text: scoreLabel
            });
            ...
            if (result.document.metadata.title) {
                itemEl.createEl('small', {
                    cls: 'ai-organiser-related-notes-preview',
            ...
            itemEl.addEventListener('mouseleave', (e) => {
                ...
                const popup = document.querySelector('.ai-organiser-related-notes-popup');
        ...
        const footerEl = this.resultContainer.createEl('div', { cls: 'ai-organiser-related-notes-footer' });
        ...
        if (this.state.results.length < 2 && this.state.folderScope !== null) {
            const hintEl = footerEl.createEl('small', {
                ...
                cls: 'ai-organiser-related-notes-few-hint'
            });
            const expandLink = hintEl.createEl('a', {
                text: ` ${t?.tryAllNotes || 'Search all notes'}`,
                cls: 'ai-organiser-related-notes-expand-link'
            });
        ...
        if (this.state.timestamp) {
            footerEl.createEl('small', {
                ...
                cls: 'ai-organiser-related-notes-timestamp'
            });
        }
    }
