...
export function stripHighlightMarkup(text: string): string {
    return text
        .replaceAll(MARK_EXTRACT, '$1')
        .replaceAll(EQUAL_EXTRACT, '$1');
}
...
function buildDisplayText(text: string, type: ContentBlockType): string {
    ...
    const clean = stripHighlightMarkup(placeholderText).replaceAll(/\s+/g, ' ').trim();
    return truncate(clean, DISPLAY_TEXT_LIMIT);
}

function replaceNonTextElements(text: string): string {
    return text.replaceAll(/!\[\[(.+?)\]\]/g, (_match, inner: string) => {
