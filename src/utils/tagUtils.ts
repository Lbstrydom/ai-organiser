...
export class TagUtils {
    ...
    /**
     * Merges two arrays of tags, removing duplicates.
     * Priority tags appear first (preserving their order), then supplementary tags
     * that are not already present. Deduplication is case-insensitive.
     * @param priorityTags - Tags that take ordering priority (e.g. new LLM tags with [theme, discipline, ...topics])
     * @param supplementaryTags - Additional tags appended after priority tags (e.g. existing tags)
     * @returns Array of unique tags with priority tags first
     */
    static mergeTags(priorityTags: string[], supplementaryTags: string[]): string[] {
        const seen = new Set<string>();
        const result: string[] = [];
        for (const tag of priorityTags.map(String)) {
            const key = tag.toLowerCase();
            if (!seen.has(key)) { seen.add(key); result.push(tag); }
        }
        for (const tag of supplementaryTags.map(String)) {
            const key = tag.toLowerCase();
            if (!seen.has(key)) { seen.add(key); result.push(tag); }
        }
        return result;
        ...
        try {
            ...
            try {
                ...
                if (!replaceTags && existingFrontmatter) {
                    ...
                    // New tags (yamlReadyTags) take priority for ordering (e.g. [theme, discipline, ...topics])
                    if (existingTags.length > 0) {
                        const combined = this.mergeTags(yamlReadyTags, existingTags);
