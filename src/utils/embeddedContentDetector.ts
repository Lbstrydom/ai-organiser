...
export type EmbeddedContentType = 'image' | 'pdf' | 'youtube' | 'web-link' | 'internal-link' | 'document' | 'audio';

/** @deprecated Use EmbeddedContentType */
export type ContentType = EmbeddedContentType;
...
function classifyInternalFile(
    ...
    let type: EmbeddedContentType = 'internal-link';
...
/**
 * Filter detected content to only Quick Peek triageable sources.
 * Excludes images (no text to triage) and internal vault links.
 */
export function getQuickPeekSources(result: DetectionResult): DetectedContent[] {
    return result.items.filter(item =>
        item.type === 'web-link' ||
        item.type === 'youtube' ||
        item.type === 'pdf' ||
 ...
 */
export function getContentTypeDisplayName(type: EmbeddedContentType): string {
    switch (type) {
 ...
 */
export function getContentTypeIcon(type: EmbeddedContentType): string {
    switch (type) {
        case 'image': return 'image';
