...
- **Consent UI (C15/C17/C18)**: `VisualSearchSettingsSection` renders INSIDE the semantic-search section UNGATED on the flag (`SECTION_HOSTED_FEATURES` SSOT in `features.ts` keeps the completeness CI invariant honest) — the pre-enable copy names all THREE transmissions (page images → embedding backend; page text → active LLM; rendered images → vision LLM); BYO key entry allowed pre-enable; the probe runs from the panel.
